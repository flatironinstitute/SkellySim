...
// Full, non-linear objective function
Eigen::VectorXd obj_generator(Eigen::VectorXd &XX, FiberChebyshevConstraint &FS, const double L) {
    // Assume the fiber generators are setup correctly for the number of equations, but don't have their state
...
// See if we can form up A by considering an input of unit matrices with the linearized objective function
TEST(FiberChebyshevConstraint, linearized_compute_A) {
...
// Test if there is any way to use autodiff to construct this
TEST(FiberChebyshevConstraint, autodiff_purpose) {
    // Construct the fiber
    int N = 16;
    FiberChebyshevConstraint FS(N, N - 3, N - 4, N - 4);
    double L = 0.1;
    ...
    // Just call once and print the result
    Eigen::VectorXd XXnew = obj_generator(XX, FS, L);
    std::cout << "XX = \n" << XX << std::endl;
    std::cout << "XXnew = \n" << XXnew << std::endl;
}