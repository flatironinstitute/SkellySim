...
def velocity(path: Path=Path('.')):
    ...
    with working_directory(path):
        ...
        # Find the max radius in XY to use for the hydrodynamic radius (for now)
        xmax = np.max(precompute_data["node_positions_ref"][:,0])
        xmin = np.min(precompute_data["node_positions_ref"][:,0])
        ymax = np.max(precompute_data["node_positions_ref"][:,1])
        ymin = np.min(precompute_data["node_positions_ref"][:,1])
        dxr = (xmax - xmin) / (2.0)
        dyr = (ymax - ymin) / (2.0)
        radius = (dxr + dyr) / (2.0)
