...
def velocity(path: Path=Path('.')):
    ...
    with working_directory(path):
        ...
        v_parallel = force / (8.0 * np.pi * eta * a * np.power(eccentricity,3) / (eccentricity*np.sqrt(1.0-np.power(eccentricity,2)) - (1.0-2.0*np.power(eccentricity,2))*np.arcsin(eccentricity)))
