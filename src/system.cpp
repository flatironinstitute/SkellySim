...
class TrajectoryReader {
  public:
    TrajectoryReader(const std::string &input_file) : offset_(0) {
        ...
        if (fd_ == -1)
            throw std::runtime_error("Unable to open trajectory file " + input_file + ".");

        struct stat sb;
        if (fstat(fd_, &sb) == -1)
            throw std::runtime_error("Error statting " + input_file + ".");

        buflen_ = sb.st_size;

        addr_ = static_cast<char *>(mmap(NULL, buflen_, PROT_READ, MAP_PRIVATE, fd_, 0u));
        if (addr_ == MAP_FAILED)
            throw std::runtime_error("Error mapping " + input_file + ".");
    }
