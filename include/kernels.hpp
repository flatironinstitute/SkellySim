...
typedef Eigen::MatrixXd (*fmm_kernel_func_t)(const Eigen::Ref<Eigen::MatrixXd> &, const Eigen::Ref<Eigen::MatrixXd> &,
                                             const Eigen::Ref<Eigen::MatrixXd> &, stkfmm::STKFMM *j);
...
Eigen::MatrixXd stokes_vel_fmm(const Eigen::Ref<Eigen::MatrixXd> &r_src, const Eigen::Ref<Eigen::MatrixXd> &r_trg,
...
class FMM {
  public:
    ...
    FMM(const int order, const int maxPoints, const stkfmm::PAXIS paxis, const stkfmm::KERNEL k, const F &kernel_func)
        : fmmPtr_(new stkfmm_type(order, maxPoints, paxis, static_cast<unsigned>(k))), kernel_func_(kernel_func){};
    ...
    Eigen::MatrixXd operator()(const Eigen::Ref<Eigen::MatrixXd> &r_src, const Eigen::Ref<Eigen::MatrixXd> &r_trg,
        ...
        return kernel_func_(r_src, r_trg, f_src, fmmPtr_.get());
    }
